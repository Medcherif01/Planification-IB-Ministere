...
export const buildUnitPlanTemplateData = (rawPlan: UnitPlan) => {
  ...
  const dataDict = {
    ...
    // ── Balises détaillées & granulaires (pour templates personnalisés ou balises avancées) ──
    chapitres:                c(plan.chapters || (plan.lessons ? plan.lessons.join(', ') : '')),
    chapitre:                 c(plan.chapters || (plan.lessons ? plan.lessons.join(', ') : '')),
    justification_concept_cle: c(plan.keyConceptJustification),
    developpement_concept_cle: c(plan.keyConceptDevelopment),
    justification_contexte_mondial: c(plan.globalContextJustification),
    aspects_contexte_mondial: c(plan.globalContextAspects),
    liens_contexte_mondial:   c(plan.globalContextLinks),
    explication_enonce:       c(plan.statementExplanation),
    transfert_enonce:         c(plan.statementTransfer),
    questions_recherche:      [...(plan.inquiryQuestions?.factual || []), ...(plan.inquiryQuestions?.conceptual || []), ...(plan.inquiryQuestions?.debatable || [])].join('\n'),
    contexte_eleves:          prerequisTxt,
    connaissances_anterieures: c(plan.studentContext?.priorKnowledge || plan.prerequisites),
    competences_acquises:     c(plan.studentContext?.acquiredSkills),
    besoins_specifiques:      c(plan.studentContext?.specificNeeds),
    difficultes_anticipees:   c(plan.studentContext?.anticipatedDifficulties),
    diversite_profils:        c(plan.studentContext?.profileDiversity),
    connaissances_details:    c(plan.contentDetails?.knowledges),
    vocabulaire:              c(plan.contentDetails?.vocabulary || plan.contentDetails?.notions),
    methodes:                 c(plan.contentDetails?.methods),
    competences_disciplinaires: c(plan.contentDetails?.disciplinarySkills),
    contenu_obligatoire:      c(plan.contentDetails?.mandatoryContent),
    liens_nationaux:          c(plan.contentDetails?.nationalLinks),
    phase1_activation:        c(plan.learningProcess?.phase1_activation),
    phase2_acquisition:       c(plan.learningProcess?.phase2_acquisition),
    phase3_pratique:          c(plan.learningProcess?.phase3_practice),
    phase3_practice:          c(plan.learningProcess?.phase3_practice),
    phase4_transfert:         c(plan.learningProcess?.phase4_transfer),
    phase4_transfer:          c(plan.learningProcess?.phase4_transfer),
    phase5_reflexion:         c(plan.learningProcess?.phase5_reflection),
    phase5_reflection:        c(plan.learningProcess?.phase5_reflection),
    seances:                  plan.sessions?.map(s => `Séance ${s.numero} (${s.duree}) : ${s.objectifApprentissage} - ${s.activite}`).join('\n') || '',
    evaluations_formatives_detaillees: plan.formativeDetails?.map(f => `${f.titre || 'Éval'} (${f.moment}) : ${f.activite} [${f.criteres}]`).join('\n') || '',
    titre_sommative:          c(plan.summativeDetails?.titre),
    contexte_sommative:       c(plan.summativeDetails?.contexte || plan.summativeDetails?.situation),
    consigne_sommative:       c(plan.summativeDetails?.consigne),
    production_sommative:     c(plan.summativeDetails?.productionAttendue),
    duree_sommative:          c(plan.summativeDetails?.duree || plan.summativeDetails?.modalites),
    modalites_sommative:      c(plan.summativeDetails?.modalites),
    ressources_sommative:     c(plan.summativeDetails?.ressourcesAutorisees),
    niveau_sommative:         c(plan.summativeDetails?.niveauAttendu),
    grille_sommative:         c(plan.summativeDetails?.grilleCriteres),
    differentiation_soutien:  c(plan.differentiationDetails?.supportStudents ? Object.values(plan.differentiationDetails.supportStudents).filter(Boolean).join('\n- ') : ''),
    differentiation_enrichissement: c(plan.differentiationDetails?.advancedStudents ? Object.values(plan.differentiationDetails.advancedStudents).filter(Boolean).join('\n- ') : ''),
